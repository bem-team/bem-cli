...
func handleCallsRetrieve(ctx context.Context, cmd *cli.Command) error {
	...
	return ShowJSON(os.Stdout, "calls retrieve", obj, format, transform)
}
...
func handleCallsList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		...
		obj := gjson.ParseBytes(res)
		return ShowJSON(os.Stdout, "calls list", obj, format, transform)
	} else {
		...
		return ShowJSONIterator(os.Stdout, "calls list", iter, format, transform, maxItems)
	}
}
